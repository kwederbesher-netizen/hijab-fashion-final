...
export default function HijabsClient({ searchParams }: { searchParams?: { search?: string; page?: string; sort?: string } }) {
  ...
  if (loading && products.length === 0) return (
    <div style={{ padding: '100px 20px', textAlign: 'center', minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <p style={{ marginTop: '20px', color: '#555', fontSize: '16px' }}>Cargando hiyabs...</p>
    </div>
  )
  ...
  const sizeChartData = {
    title: 'Guía de Tallas - Hiyabs Turcos',
    columns: ['Talla', 'Busto (cm)', 'Cintura (cm)', 'Cadera (cm)', 'Largo (cm)'],
    rows: [
      { size: 'S', bust: '84-88', waist: '66-70', hip: '90-94', length: '180-185' },
      { size: 'M', bust: '88-92', waist: '70-74', hip: '94-98', length: '182-187' },
      { size: 'L', bust: '92-96', waist: '74-78', hip: '98-102', length: '184-189' },
      { size: 'XL', bust: '96-100', waist: '78-82', hip: '102-106', length: '186-191' },
      { size: '2XL', bust: '100-104', waist: '82-86', hip: '106-110', length: '188-193' },
      { size: '3XL', bust: '104-108', waist: '86-90', hip: '110-114', length: '190-195' },
      { size: '4XL', bust: '108-112', waist: '90-94', hip: '114-118', length: '192-197' }
    ]
  }
  ...
  return (
    <>
      ...
      <section style={{ background: 'linear-gradient(135deg, #fdf8f2 0%, #ffffff 100%)', padding: '60px 0 40px', textAlign: 'center', borderBottom: '1px solid #f0e6dc', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', fontSize: '200px', fontWeight: 800, color: 'rgba(255,90,0,0.03)', whiteSpace: 'nowrap', zIndex: 0, fontFamily: 'Playfair Display, serif' }}>HIJABS</div>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', flexWrap: 'wrap', gap: '15px', background: 'white', padding: '15px 25px', borderRadius: '20px', boxShadow: '0 5px 15px rgba(0,0,0,0.03)', border: '1px solid #f0e6dc' }}>
          <div style={{ color: '#5a4a3a', fontSize: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span><span style={{ fontWeight: 600, color: '#ff5a00' }}>{totalProducts}</span> hiyabs referenciados</span>
        ...
        {products.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '80px 20px', background: 'white', borderRadius: '20px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)', border: '1px solid #f0e6dc' }}>
            ...
            <h3 style={{ fontSize: '24px', color: '#2c2418', marginBottom: '10px', fontWeight: 600 }}>No se encontraron hiyabs</h3>
            ...
            {loading && products.length > 0 ? <LoadingSkeleton /> : (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '25px', marginBottom: '50px' }}>
                {products.map((p: any) => {
                  ...
                  else if (fabric.includes('silk') || fabric.includes('seda')) fabricBadge = <span style={{ background: '#d48c54', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>SEDA</span>
                  ...
                  else if (fabric.includes('chiffon') || fabric.includes('gasa')) fabricBadge = <span style={{ background: '#c98a60', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>GASA</span>
                  ...
                  return (
                    <Link href={getProductUrl(p)} key={p._id} style={{ background: 'white', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 5px 20px rgba(0,0,0,0.05)', transition: 'all 0.3s', textDecoration: 'none', color: 'inherit', border: '1px solid #f0e6dc', display: 'block' }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-8px)'; e.currentTarget.style.boxShadow = '0 15px 30px rgba(255,90,0,0.12)'; e.currentTarget.style.borderColor = '#ff5a00'; }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 5px 20px rgba(0,0,0,0.05)'; e.currentTarget.style.borderColor = '#f0e6dc'; }}>
                      <div style={{ position: 'relative', height: '280px', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #fef9f2 0%, #faf5ed 100%)' }}>
                        <img src={getProductImage(p.mainImage, p.imageUrl, { width: 400 }, p.images)} alt={p.name_es || p.name_en || p.name_ar || 'Hiyab turco'} style={{ maxWidth: '90%', maxHeight: '90%', objectFit: 'contain', transition: 'transform 0.5s' }} loading="lazy" onError={(e) => { (e.target as HTMLImageElement).src = '/images/default.webp' }} />
                      ...
                      <div style={{ padding: '20px', textAlign: 'center' }}>
                        <div style={{ fontSize: '11px', color: '#ff5a00', marginBottom: '8px', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '1px' }}>Hiyab</div>
      ...
      <section style={{ background: '#fef9f2', padding: '60px 0', borderTop: '1px solid #f0e6dc', marginTop: '40px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '32px', color: '#2c2418', marginBottom: '20px', fontWeight: 700, textAlign: 'center', fontFamily: 'Playfair Display, serif' }}>Colección Completa de Hiyabs Turcos al por Mayor</h2>
            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', marginBottom: '25px', textAlign: 'justify' }}>
              Bienvenido a Hijab Fashion Mall, su <strong>mayorista de confianza para hiyabs turcos</strong> desde 2019. 
              Nuestra colección al por mayor ofrece más de <strong>{totalProducts} modelos de hiyabs</strong> adaptados a las necesidades 
            ...
            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', textAlign: 'justify' }}>
              Los <strong>hiyabs turcos</strong> se han convertido en un referente mundial por su calidad excepcional, la suavidad de sus tejidos 
              y la variedad de colores disponibles. Nuestra colección al por mayor incluye hiyabs de modal, seda líquida, jersey suave y gasa ligera.
              Para pedidos al por mayor o consultas personalizadas, contáctanos vía WhatsApp o Telegram. Envío mundial garantizado con seguimiento.
            </p>
          </div>
        </div>
      </section>
