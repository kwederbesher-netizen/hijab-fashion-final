...
export default function ModestSportswearClient({ searchParams }: { searchParams?: { search?: string; page?: string; sort?: string } }) {
  ...
  const sizeChartData = {
    title: 'Guía de Tallas - Ropa Deportiva',
    ...
    rows: [
      ...
      { size: '2XL', bust: '100-104', waist: '82-86', hip: '106-110' },
      { size: '3XL', bust: '104-108', waist: '86-90', hip: '110-114' },
      { size: '4XL', bust: '108-112', waist: '90-94', hip: '114-118' }
    ]
  }
      ...
      {showSizeChart && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={() => setShowSizeChart(false)}>
          <div style={{ background: 'white', borderRadius: '24px', maxWidth: '650px', width: '100%', padding: '30px' }} onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{ fontSize: '12px', color: '#a08c7a', marginTop: '15px', textAlign: 'center' }}>Telas con elastano para mayor comodidad durante el ejercicio.</p>
          </div>
        </div>
      )}
                  ...
                  return (
                    <Link href={getProductUrl(p)} key={p._id} style={{ background: 'white', borderRadius: '16px', overflow: 'hidden', textDecoration: 'none', color: 'inherit', border: '1px solid #f0e6dc', transition: 'all 0.3s' }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-8px)'; e.currentTarget.style.boxShadow = '0 15px 30px rgba(255,90,0,0.12)'; e.currentTarget.style.borderColor = '#ff5a00'; }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 5px 20px rgba(0,0,0,0.05)'; e.currentTarget.style.borderColor = '#f0e6dc'; }}>
                      <div style={{ position: 'relative', height: '280px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #fef9f2 0%, #faf5ed 100%)' }}>
                        <img src={getProductImage(p.mainImage, p.imageUrl, { width: 400 }, p.images)} alt={p.name_es || p.name_en || 'Ropa deportiva'} style={{ maxWidth: '90%', maxHeight: '90%', objectFit: 'contain' }} loading="lazy" onError={(e) => { (e.target as HTMLImageElement).src = '/images/default.webp' }} />
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            ...
            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', textAlign: 'justify' }}>
              Ofrecemos conjuntos de chándal, leggings anchos para entrenamiento y yoga, sudaderas largas que cubren cadera, y camisetas deportivas de mangas largas. Todos los productos están disponibles en tallas S hasta 4XL. Para pedidos al por mayor, contáctanos vía WhatsApp o Telegram.
            </p>
          </div>
        </div>
      </section>
