...
export default function ModestSportswearCategoryPageFr() {
  ...
  useEffect(() => {
    if (!cartInitialized.current) {
      ...
      if (savedCart) {
        try { const parsedCart = JSON.parse(savedCart); setCart(parsedCart); cartStringRef.current = JSON.stringify(parsedCart) } catch (e) {}
      }
  ...
  useEffect(() => {
    const handleCartUpdate = () => {
      ...
      const savedCart = localStorage.getItem('cart')
      if (savedCart) {
        try { const parsedCart = JSON.parse(savedCart); const newCartString = JSON.stringify(parsedCart); if (newCartString !== cartStringRef.current) { setCart(parsedCart); cartStringRef.current = newCartString } } catch (e) {}
      }
  ...
  const getColorCode = useCallback((color: string) => {
    const colors: any = { 'Black': '#1a1a1a', 'White': '#FFFFFF', 'Navy': '#0a192f', 'Gray': '#6b6b6b', 'Burgundy': '#6e2c2c', 'Green': '#2d6a4f', 'Blue': '#1e40af', 'Pink': '#d47a8a', 'Purple': '#5a2d6e', 'Beige': '#d4b896' }
  ...
  if (loading && products.length === 0) return (
    <div style={{ padding: '100px 20px', textAlign: 'center', minHeight: '60vh' }}>
      ...
      <p style={{ marginTop: '20px', color: '#5a4a3a' }}>Chargement des tenues de sport...</p>
    </div>
  )
  ...
  return (
    <>
      <Head>
        <title>Tenues de Sport Modestes Turques en Gros | Sportswear | Hijab Fashion Mall</title>
        <meta name="description" content={`Découvrez ${totalProducts}+ tenues de sport modestes turques en gros. Leggings, hoodies, sweats, joggings. Tissus techniques respirants. Livraison mondiale.`} />
        <meta name="keywords" content="tenue de sport modeste en gros, grossiste sportswear, leggings modestes, hoodie modeste, jogging femme musulmane, vêtement sport hijab, sportswear turc, fitness modeste, running hijab, gym modeste, survêtement musulman, tenue de sport couvrante" />
        <meta name="robots" content="index, follow" />
        <link rel="canonical" href="https://hijabfashionmall.com/fr/category/modest-sportswear" />
        ...
        <meta property="og:title" content="Tenues de Sport Modestes Turques en Gros - Sportswear" />
        <meta property="og:description" content={`${totalProducts}+ tenues de sport premium. Tissus techniques respirants.`} />
      ...
      <section style={{ background: 'linear-gradient(135deg, #fdf8f2 0%, #ffffff 100%)', padding: '60px 0 40px', textAlign: 'center', borderBottom: '1px solid #f0e6dc', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', fontSize: '200px', fontWeight: 800, color: 'rgba(255,90,0,0.03)', whiteSpace: 'nowrap', zIndex: 0, fontFamily: 'Playfair Display, serif' }}>SPORT</div>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px', position: 'relative', zIndex: 1 }}>
          <div style={{ fontSize: '14px', color: '#7a6a5a', marginBottom: '20px' }}>
            ...
            <span style={{ color: '#7a6a5a' }}>Tenues de Sport</span>
          </div>
          <h1 style={{ fontSize: '48px', color: '#2c2418', marginBottom: '15px', fontWeight: 800, fontFamily: 'Playfair Display, serif' }}>Tenues de Sport <span style={{ color: '#ff5a00' }}>Modestes</span></h1>
          <p style={{ fontSize: '18px', color: '#5a4a3a', maxWidth: '800px', margin: '0 auto', lineHeight: '1.6' }}>Découvrez notre collection de tenues de sport modestes turques en gros. Leggings, hoodies, sweats. Tissus techniques respirants pour un confort optimal pendant l'effort.</p>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '40px', marginTop: '40px', flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <span style={{ color: '#4a3a2a' }}>Tissus Respirants</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <span style={{ color: '#4a3a2a' }}>Confortables</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <span style={{ color: '#4a3a2a' }}>Élastiques</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <main>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', flexWrap: 'wrap', gap: '15px', background: 'white', padding: '15px 25px', borderRadius: '20px', boxShadow: '0 5px 15px rgba(0,0,0,0.03)', border: '1px solid #f0e6dc' }}>
              <div style={{ color: '#5a4a3a', display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span><span style={{ fontWeight: 600, color: '#ff5a00' }}>{totalProducts}</span> tenues référencées</span>
            ...
            {products.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '80px 20px', background: 'white', borderRadius: '20px', border: '1px solid #f0e6dc' }}>
                ...
                <h3 style={{ fontSize: '24px', color: '#2c2418' }}>Aucune tenue trouvée</h3>
                    ...
                    {products.map((p: any) => {
                      ...
                      return (
                        <Link href={getProductUrl(p)} key={p._id} style={{ background: 'white', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 5px 20px rgba(0,0,0,0.05)', transition: 'all 0.3s', textDecoration: 'none', color: 'inherit', border: '1px solid #f0e6dc', display: 'block' }}>
                          ...
                          <div style={{ padding: '20px', textAlign: 'center' }}>
                            <div style={{ fontSize: '11px', color: '#ff5a00', marginBottom: '8px', textTransform: 'uppercase' }}>Sportswear</div>
      ...
      <section style={{ background: '#fef9f2', padding: '60px 0', borderTop: '1px solid #f0e6dc' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '32px', color: '#2c2418', textAlign: 'center', fontFamily: 'Playfair Display, serif' }}>Guide des Tailles - Sportswear Turc</h2>
            <div style={{ overflowX: 'auto', marginTop: '30px' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', background: 'white', borderRadius: '20px', overflow: 'hidden' }}>
                ...
                <tbody>
                  {[['S', '82-86', '64-68', '88-92', '65'], ['M', '86-90', '68-72', '92-96', '67'], ['L', '90-94', '72-76', '96-100', '69'], ['XL', '94-98', '76-80', '100-104', '71'], ['2XL', '98-102', '80-84', '104-108', '73'], ['3XL', '102-106', '84-88', '108-112', '75']].map((row, idx) => (
