// app/fr/category/modest-evening-dresses/ModestEveningDressesClient.tsx
'use client'
...
export default function ModestEveningDressesClient({ searchParams }: { searchParams?: { search?: string; page?: string; sort?: string } }) {
  ...
  const getColorCode = useCallback((color: string) => {
    const colors: any = {
      ...
      'Pink': '#d47a8a', 'Orange': '#e86a2c', 'Champagne': '#f7e8c8',
      'Gold': '#d4a83a', 'Silver': '#c0c0c0', 'Emerald': '#2e8b57',
      'Royal Blue': '#4169e1', 'Dusty Rose': '#c9a0a8', 'Lavender': '#b8a9c9'
    }
  ...
  if (loading && products.length === 0) return (
    <div style={{ 
      ...
      `}</style>
      <p style={{ marginTop: '20px', color: '#5a4a3a', fontSize: '16px' }}>Chargement des robes de soirée...</p>
    </div>
  )
      ...
      {showSizeChart && (
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{ fontSize: '28px', color: '#2c2418', marginBottom: '20px', textAlign: 'center', fontFamily: 'Playfair Display, serif' }}>Guide des Tailles - Robes de Soirée Turques</h2>
            <p style={{ textAlign: 'center', color: '#5a4a3a', marginBottom: '30px' }}>Les tailles suivantes correspondent aux standards turcs pour les robes de soirée.</p>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {[['S','84-88','66-70','90-94','140-150'],['M','88-92','70-74','94-98','142-152'],['L','92-96','74-78','98-102','144-154'],['XL','96-100','78-82','102-106','146-156'],['2XL','100-104','82-86','106-110','148-158'],['3XL','104-108','86-90','110-114','150-160'],['4XL','108-112','90-94','114-118','152-162']].map((row,i)=>(
      ...
      <section style={{
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px', position: 'relative', zIndex: 1 }}>
          <div style={{ fontSize: '14px', color: '#7a6a5a', marginBottom: '20px' }}>
            ...
            <span style={{ margin: '0 8px' }}>/</span>
            <span style={{ color: '#7a6a5a' }}>Robes de Soirée</span>
          </div>
          <h1 style={{ 
            fontSize: '48px', 
            color: '#2c2418', 
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <span style={{ color: '#4a3a2a' }}>Mariage & Réception</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <span style={{ color: '#4a3a2a' }}>Tissus Luxueux</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <span style={{ color: '#4a3a2a' }}>Broderies & Dentelle</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                {loading && products.length > 0 ? <LoadingSkeleton /> : (
                  <div style={{
                    ...
                    {products.map((p: any) => {
                      ...
                      let dressBadge = null
                      if (dressName.includes('wedding') || dressName.includes('mariage')) dressBadge = <span style={{ background: '#b87c4f', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>MARIAGE</span>
                      else if (dressName.includes('engagement') || dressName.includes('fiançailles')) dressBadge = <span style={{ background: '#d48c54', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>FIANÇAILLES</span>
                      else if (dressName.includes('lace') || dressName.includes('dentelle')) dressBadge = <span style={{ background: '#c98a60', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>DENTELLE</span>
                      else if (dressName.includes('embroidered') || dressName.includes('brodé')) dressBadge = <span style={{ background: '#b07d4a', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>BRODERIE</span>
                      ...
                      return (
                        <Link href={getProductUrl(p)} key={p._id} style={{ background: 'white', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 5px 20px rgba(0,0,0,0.05)', transition: 'all 0.3s', textDecoration: 'none', color: 'inherit', border: '1px solid #f0e6dc', display: 'block' }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-8px)'; e.currentTarget.style.borderColor = '#ff5a00' }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = '#f0e6dc' }}>
                          <div style={{ position: 'relative', height: '280px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #fef9f2 0%, #faf5ed 100%)' }}>
                            <img src={getProductImage(p.mainImage, p.imageUrl, { width: 400 }, p.images)} alt={p.name_fr || p.name_en || 'Robe de soirée turque'} style={{ maxWidth: '90%', maxHeight: '90%', objectFit: 'contain' }} loading="lazy" onError={(e) => { (e.target as HTMLImageElement).src = '/images/default.webp' }} />
                          ...
                          <div style={{ padding: '20px', textAlign: 'center' }}>
                            <div style={{ fontSize: '11px', color: '#ff5a00', marginBottom: '8px', textTransform: 'uppercase' }}>Robe de Soirée</div>
      ...
      <section style={{ background: '#fef9f2', padding: '60px 0', borderTop: '1px solid #f0e6dc' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '32px', color: '#2c2418', textAlign: 'center', fontFamily: 'Playfair Display, serif' }}>Collection Robes de Soirée Modestes Turques en Gros</h2>
            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', textAlign: 'justify' }}>Découvrez notre collection de <strong>robes de soirée modestes turques en gros</strong>. Robes de cérémonie, mariage, réception. Tissus luxueux, broderies, dentelle. Pour toute commande en gros, contactez notre équipe via WhatsApp ou Telegram. Livraison mondiale.</p>
          </div>
        </div>
      </section>
