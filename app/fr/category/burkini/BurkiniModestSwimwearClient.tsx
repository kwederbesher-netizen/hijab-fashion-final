// app/fr/category/burkini-modest-swimwear/BurkiniModestSwimwearClient.tsx
'use client'
...
export default function BurkiniModestSwimwearClient({ searchParams }: { searchParams?: { search?: string; page?: string; sort?: string } }) {
  ...
  const getColorCode = useCallback((color: string) => {
    const colors: any = {
      ...
      'Gray': '#6b6b6b', 'Burgundy': '#6e2c2c', 'Blue': '#1e40af',
      'Green': '#2d6a4f', 'Teal': '#2c6e6e', 'Purple': '#5a2d6e',
      'Pink': '#d47a8a', 'Coral': '#e87a5e', 'Mint Green': '#6fbf8f'
    }
  ...
  if (loading && products.length === 0) return (
    <div style={{ 
      ...
      `}</style>
      <p style={{ marginTop: '20px', color: '#5a4a3a', fontSize: '16px' }}>Chargement des burkinis...</p>
    </div>
  )
      ...
      {showSizeChart && (
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{ fontSize: '28px', color: '#2c2418', marginBottom: '20px', textAlign: 'center', fontFamily: 'Playfair Display, serif' }}>Guide des Tailles - Burkini Turc</h2>
            <p style={{ textAlign: 'center', color: '#5a4a3a', marginBottom: '30px' }}>Les tailles suivantes correspondent aux standards turcs pour les burkinis.</p>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {[['S','82-86','64-68','88-92','130'],['M','86-90','68-72','92-96','132'],['L','90-94','72-76','96-100','134'],['XL','94-98','76-80','100-104','136'],['2XL','98-102','80-84','104-108','138'],['3XL','102-106','84-88','108-112','140'],['4XL','106-110','88-92','112-116','142']].map((row,i)=>(
      ...
      <section style={{
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px', position: 'relative', zIndex: 1 }}>
          ...
          <h1 style={{ 
            ...
            Burkini <span style={{ color: '#ff5a00' }}>Turc</span>
          </h1>
          <p style={{ fontSize: '18px', color: '#5a4a3a', maxWidth: '800px', margin: '0 auto', lineHeight: '1.6' }}>
            Découvrez notre collection de burkinis turcs en gros. Maillots de bain modestes 2 ou 3 pièces, 
            tissus résistants à l'eau, séchage rapide. Parfaits pour la plage et la piscine.
          </p>
          ...
          <main>
            ...
            {products.length === 0 ? (
              <div style={{
                ...
                <h3 style={{ fontSize: '24px', color: '#2c2418', marginBottom: '10px', fontWeight: 600 }}>Aucun burkini trouvé</h3>
                      ...
                      return (
                        <Link href={getProductUrl(p)} key={p._id} style={{ background: 'white', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 5px 20px rgba(0,0,0,0.05)', transition: 'all 0.3s', textDecoration: 'none', color: 'inherit', border: '1px solid #f0e6dc', display: 'block' }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-8px)'; e.currentTarget.style.borderColor = '#ff5a00' }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = '#f0e6dc' }}>
                          <div style={{ position: 'relative', height: '280px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #fef9f2 0%, #faf5ed 100%)' }}>
                            <img src={getProductImage(p.mainImage, p.imageUrl, { width: 400 }, p.images)} alt={p.name_fr || p.name_en || 'Burkini turc'} style={{ maxWidth: '90%', maxHeight: '90%', objectFit: 'contain' }} loading="lazy" onError={(e) => { (e.target as HTMLImageElement).src = '/images/default.webp' }} />
      ...
      <section style={{ background: '#fef9f2', padding: '60px 0', borderTop: '1px solid #f0e6dc' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '32px', color: '#2c2418', textAlign: 'center', fontFamily: 'Playfair Display, serif' }}>Burkini Turc en Gros - Maillot de Bain Modeste</h2>
            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', textAlign: 'justify' }}>Découvrez notre collection de <strong>burkinis turcs en gros</strong>. Maillots de bain modestes 2 ou 3 pièces, tissus résistants à l'eau, séchage rapide. Parfaits pour la plage et la piscine. Pour toute commande en gros, contactez notre équipe via WhatsApp ou Telegram. Livraison mondiale.</p>
          </div>
        </div>
      </section>
