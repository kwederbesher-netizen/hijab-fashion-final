// app/it/category/abayas/page.tsx
'use client'
...
export default function AbayasCategoryPageIt() {
  ...
  // Size chart component for Abayas
  const SizeChart = () => {
    return (
      <div style={{
        marginTop: '40px',
        ...
        <p style={{
          ...
          ⚠️ Le misure sono indicative e possono variare di 1-2 cm a seconda del modello e del tessuto.<br />
          Per abaya elasticizzate o con cintura, si consiglia di scegliere la taglia abituale.
        </p>
      </div>
    )
  }
  ...
  if (loading && products.length === 0) return (
    <div style={{ padding: '100px 20px', textAlign: 'center', minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <p style={{ marginTop: '20px', color: '#555', fontSize: '16px' }}>Caricamento abaya...</p>
    </div>
  )
  ...
  return (
    <>
      <Head>
        <title>Abaya Turche all'Ingrosso | Collezione Premium Seta Medina, Crêpe, Jersey | Hijab Fashion Mall</title>
        <meta name="description" content={`Scopri ${totalProducts}+ abaya turche all'ingrosso. Seta di Medina fluida, crêpe elegante, jersey morbido, pizzo raffinato. Tagli moderni, taglie fino al 4XL.`} />
        <meta name="keywords" content="abaya turche all'ingrosso, grossista abaya, abaya seta medina, abaya crêpe, abaya jersey, abaya pizzo, abaya aperta, abaya chiusa, abaya taglie forti, abaya moderna" />
        <meta name="robots" content="index, follow" />
        <link rel="canonical" href="https://hijabfashionmall.com/it/category/abayas" />
        ...
        <link rel="alternate" href="https://hijabfashionmall.com/it/category/abayas" hrefLang="it" />
        
        <meta property="og:title" content="Collezione Abaya Turche all'Ingrosso - Seta Medina & Crêpe" />
        <meta property="og:description" content={`${totalProducts}+ modelli di abaya premium. Tessuti d'eccezione.`} />
        <meta property="og:url" content="https://hijabfashionmall.com/it/category/abayas" />
        <meta property="og:type" content="website" />
        <meta property="og:image" content="https://hijabfashionmall.com/images/og-abayas-it.jpg" />
      ...
      <section style={{ background: 'linear-gradient(135deg, #fdf8f2 0%, #ffffff 100%)', padding: '60px 0 40px', textAlign: 'center', borderBottom: '1px solid #f0e6dc', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', fontSize: '180px', fontWeight: 800, color: 'rgba(255,90,0,0.03)', whiteSpace: 'nowrap', zIndex: 0, pointerEvents: 'none', fontFamily: 'Playfair Display, serif' }}>ABAYA</div>
          ...
          <main>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', flexWrap: 'wrap', gap: '15px', background: 'white', padding: '15px 25px', borderRadius: '20px', boxShadow: '0 5px 15px rgba(0,0,0,0.03)', border: '1px solid #f0e6dc' }}>
              <div style={{ color: '#5a4a3a', fontSize: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span><span style={{ fontWeight: 600, color: '#ff5a00' }}>{totalProducts}</span> abaya</span>
            ...
            {products.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '80px 20px', background: 'white', borderRadius: '20px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)', border: '1px solid #f0e6dc' }}>
                ...
                <h3 style={{ fontSize: '24px', color: '#2c2418', marginBottom: '10px', fontWeight: 600 }}>Nessuna abaya trovata</h3>
                      ...
                      return (
                        <Link href={getProductUrl(p)} key={p._id} style={{ background: 'white', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 5px 20px rgba(0,0,0,0.05)', transition: 'all 0.3s', textDecoration: 'none', color: 'inherit', border: '1px solid #f0e6dc', display: 'block' }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-8px)'; e.currentTarget.style.boxShadow = '0 15px 30px rgba(255,90,0,0.12)'; e.currentTarget.style.borderColor = '#ff5a00'; }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 5px 20px rgba(0,0,0,0.05)'; e.currentTarget.style.borderColor = '#f0e6dc'; }}>
                          <div style={{ position: 'relative', height: '280px', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #fef9f2 0%, #faf5ed 100%)' }}>
                            <img src={p.imageUrl || '/images/default.webp'} alt={p.name_it || p.name_en || p.name_ar || 'Abaya turca'} style={{ maxWidth: '90%', maxHeight: '90%', objectFit: 'contain', transition: 'transform 0.5s' }} loading="lazy" onError={(e) => { (e.target as HTMLImageElement).src = '/images/default.webp' }} />
      ...
      <section style={{ background: '#fef9f2', padding: '60px 0', borderTop: '1px solid #f0e6dc', marginTop: '40px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '32px', color: '#2c2418', marginBottom: '20px', fontWeight: 700, textAlign: 'center', fontFamily: 'Playfair Display, serif' }}>Abaya Turche all'Ingrosso</h2>
            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', marginBottom: '25px', textAlign: 'justify' }}>
              Hijab Fashion Mall offre una selezione di <strong>abaya turche all'ingrosso</strong> con oltre <strong>{totalProducts} modelli</strong> disponibili. 
              Tessuti selezionati come seta di Medina, crêpe, jersey e pizzo. Taglie dalla S alla 4XL.
            </p>
            
            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>Tipi di Abaya Disponibili</h3>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '35px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}>
                ...
                <span style={{ color: '#4a3a2a' }}><strong>Abaya in Seta di Medina</strong> – Tessuto lussuoso, caduta fluida</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg>
                <span style={{ color: '#4a3a2a' }}><strong>Abaya in Crêpe</strong> – Tessuto resistente, anti-piega</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg>
                <span style={{ color: '#4a3a2a' }}><strong>Abaya in Jersey</strong> – Morbido, elasticizzato, confortevole</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg>
                <span style={{ color: '#4a3a2a' }}><strong>Abaya in Pizzo</strong> – Dettagli delicati per cerimonie</span>
              </div>
            </div>
