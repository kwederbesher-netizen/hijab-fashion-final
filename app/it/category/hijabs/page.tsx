// app/it/category/hijabs/page.tsx
'use client'
...
export default function HijabsCategoryPageIt() {
  ...
  // Size chart for Hijabs (dimensioni standard)
  const SizeChart = () => {
    return (
      <div style={{
        marginTop: '40px',
        ...
        <h4 style={{
          ...
          Dimensioni Hijab - Tessuti Turchi
        </h4>
        <table style={{
          width: '100%',
          ...
          <thead>
            <tr style={{ background: '#fef9f2', borderBottom: '2px solid #f0e6dc' }}>
              <th style={{ padding: '12px', textAlign: 'center', color: '#ff5a00', fontWeight: 700 }}>Tipo Hijab</th>
              <th style={{ padding: '12px', textAlign: 'center', color: '#ff5a00', fontWeight: 700 }}>Lunghezza (cm)</th>
              <th style={{ padding: '12px', textAlign: 'center', color: '#ff5a00', fontWeight: 700 }}>Larghezza (cm)</th>
              <th style={{ padding: '12px', textAlign: 'center', color: '#ff5a00', fontWeight: 700 }}>Peso (g)</th>
             </tr>
          </thead>
          <tbody>
            {[
              { tipo: 'Chiffon', lunghezza: '180', larghezza: '70-90', peso: '80-100' },
              { tipo: 'Seta Medina', lunghezza: '180-200', larghezza: '70-90', peso: '120-150' },
              { tipo: 'Jersey', lunghezza: '180', larghezza: '70-80', peso: '150-180' },
              { tipo: 'Georgette', lunghezza: '180', larghezza: '70-90', peso: '90-110' },
              { tipo: 'Cotone Egiziano', lunghezza: '180', larghezza: '70-80', peso: '130-160' },
              { tipo: 'Viscosa', lunghezza: '180', larghezza: '70-80', peso: '110-140' },
            ].map((size, idx) => (
              <tr key={idx} style={{ borderBottom: '1px solid #f0e6dc' }}>
                <td style={{ padding: '10px', textAlign: 'center', fontWeight: 600, color: '#2c2418' }}>{size.tipo}</td>
                <td style={{ padding: '10px', textAlign: 'center', color: '#5a4a3a' }}>{size.lunghezza}</td>
                <td style={{ padding: '10px', textAlign: 'center', color: '#5a4a3a' }}>{size.larghezza}</td>
                <td style={{ padding: '10px', textAlign: 'center', color: '#5a4a3a' }}>{size.peso}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <p style={{
          ...
          ⚠️ Le dimensioni possono variare leggermente a seconda del modello e del tessuto.
        </p>
      </div>
    )
  }
  ...
  if (loading && products.length === 0) return (
    <div style={{ padding: '100px 20px', textAlign: 'center', minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <p style={{ marginTop: '20px', color: '#555', fontSize: '16px' }}>Caricamento hijab...</p>
    </div>
  )
  ...
  return (
    <>
      <Head>
        <title>Hijab all'Ingrosso | Veli in Chiffon, Seta Medina, Jersey | Hijab Fashion Mall</title>
        <meta name="description" content={`Scopri ${totalProducts}+ hijab all'ingrosso. Veli in chiffon, seta di Medina, jersey, georgette, cotone egiziano. Colori e stili vari. Acquisto online per professionisti.`} />
        <meta name="keywords" content="hijab all'ingrosso, grossista hijab, velo modesto, chiffon hijab, hijab seta medina, hijab jersey, hijab georgette, hijab cotone, foulard islamico, hijab turco, hijab arabo, hijab quotidiano, hijab cerimonia, wholesale hijab, hijab professionali, moda modesta hijab, collezione hijab 2026" />
        <meta name="robots" content="index, follow" />
        <link rel="canonical" href="https://hijabfashionmall.com/it/category/hijabs" />
        ...
        <link rel="alternate" href="https://hijabfashionmall.com/it/category/hijabs" hrefLang="it" />
        
        <meta property="og:title" content="Hijab all'Ingrosso - Chiffon, Seta Medina & Jersey" />
        <meta property="og:description" content={`${totalProducts}+ modelli di hijab premium. Tessuti: chiffon, seta Medina, jersey, georgette.`} />
        <meta property="og:url" content="https://hijabfashionmall.com/it/category/hijabs" />
        <meta property="og:type" content="website" />
        <meta property="og:image" content="https://hijabfashionmall.com/images/og-hijabs-it.jpg" />
      ...
      <section style={{ background: 'linear-gradient(135deg, #fdf8f2 0%, #ffffff 100%)', padding: '60px 0 40px', textAlign: 'center', borderBottom: '1px solid #f0e6dc', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', fontSize: '180px', fontWeight: 800, color: 'rgba(255,90,0,0.03)', whiteSpace: 'nowrap', zIndex: 0, pointerEvents: 'none', fontFamily: 'Playfair Display, serif' }}>HIJAB</div>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px', position: 'relative', zIndex: 1 }}>
          <div style={{ fontSize: '14px', color: '#7a6a5a', marginBottom: '20px' }}>
            ...
            <span style={{ color: '#7a6a5a', margin: '0 8px' }}>/</span>
            <span style={{ color: '#7a6a5a' }}>Hijab</span>
          </div>
          <h1 style={{ fontSize: '48px', color: '#2c2418', marginBottom: '15px', fontWeight: 800, lineHeight: '1.2', fontFamily: 'Playfair Display, serif' }}>
            Hijab <span style={{ color: '#ff5a00' }}>all'Ingrosso</span>
          </h1>
          <p style={{ fontSize: '18px', color: '#5a4a3a', maxWidth: '800px', margin: '0 auto', lineHeight: '1.6' }}>
            Scopri la nostra collezione di hijab all'ingrosso. Veli in chiffon, seta di Medina, jersey, 
            georgette e cotone egiziano. Tessuti pregiati per la tua clientela.
          </p>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '40px', marginTop: '40px', flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>Chiffon & Georgette</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>Seta di Medina</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>Jersey & Viscosa</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>Cotone Egiziano</span>
            </div>
          </div>
        </div>
      </section>
          ...
          <main>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', flexWrap: 'wrap', gap: '15px', background: 'white', padding: '15px 25px', borderRadius: '20px', boxShadow: '0 5px 15px rgba(0,0,0,0.03)', border: '1px solid #f0e6dc' }}>
              <div style={{ color: '#5a4a3a', fontSize: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span><span style={{ fontWeight: 600, color: '#ff5a00' }}>{totalProducts}</span> hijab</span>
            ...
            {products.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '80px 20px', background: 'white', borderRadius: '20px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)', border: '1px solid #f0e6dc' }}>
                ...
                <h3 style={{ fontSize: '24px', color: '#2c2418', marginBottom: '10px', fontWeight: 600 }}>Nessun hijab trovato</h3>
                ...
                {loading && products.length > 0 ? <LoadingSkeleton /> : (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '25px', marginBottom: '50px' }}>
                    {products.map((p: any) => {
                      ...
                      if (fabric.includes('chiffon')) fabricBadge = <span style={{ background: '#b87c4f', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>CHIFFON</span>
                      else if (fabric.includes('medina') || fabric.includes('silk')) fabricBadge = <span style={{ background: '#d48c54', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>SETA MEDINA</span>
                      ...
                      else if (fabric.includes('georgette')) fabricBadge = <span style={{ background: '#c98a60', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>GEORGETTE</span>
                      else if (fabric.includes('cotton')) fabricBadge = <span style={{ background: '#8b6b3d', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>COTONE</span>
                      ...
                      return (
                        <Link href={getProductUrl(p)} key={p._id} style={{ background: 'white', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 5px 20px rgba(0,0,0,0.05)', transition: 'all 0.3s', textDecoration: 'none', color: 'inherit', border: '1px solid #f0e6dc', display: 'block' }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-8px)'; e.currentTarget.style.boxShadow = '0 15px 30px rgba(255,90,0,0.12)'; e.currentTarget.style.borderColor = '#ff5a00'; }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 5px 20px rgba(0,0,0,0.05)'; e.currentTarget.style.borderColor = '#f0e6dc'; }}>
                          <div style={{ position: 'relative', height: '280px', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #fef9f2 0%, #faf5ed 100%)' }}>
                            <img src={p.imageUrl || '/images/default.webp'} alt={p.name_it || p.name_en || p.name_ar || 'Hijab turco'} style={{ maxWidth: '90%', maxHeight: '90%', objectFit: 'contain', transition: 'transform 0.5s' }} loading="lazy" onError={(e) => { (e.target as HTMLImageElement).src = '/images/default.webp' }} />
      ...
      <section style={{ background: '#fef9f2', padding: '60px 0', borderTop: '1px solid #f0e6dc', marginTop: '40px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '32px', color: '#2c2418', marginBottom: '20px', fontWeight: 700, textAlign: 'center', fontFamily: 'Playfair Display, serif' }}>Hijab Turchi all'Ingrosso</h2>
            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', marginBottom: '25px', textAlign: 'justify' }}>
              Hijab Fashion Mall offre una selezione di <strong>hijab all'ingrosso</strong> con oltre <strong>{totalProducts} modelli</strong> disponibili. 
              Tessuti selezionati come chiffon, seta di Medina, jersey, georgette e cotone egiziano. 
              Colori basic e stagionali per soddisfare ogni esigenza.
            </p>
            
            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>Tipi di Hijab Disponibili</h3>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '35px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}>
                ...
                <span style={{ color: '#4a3a2a' }}><strong>Hijab in Chiffon</strong> – Leggero e traspirante</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg>
                <span style={{ color: '#4a3a2a' }}><strong>Hijab in Seta di Medina</strong> – Tessuto lussuoso</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg>
                <span style={{ color: '#4a3a2a' }}><strong>Hijab in Jersey</strong> – Morbido, non scivola</span>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg>
                <span style={{ color: '#4a3a2a' }}><strong>Hijab in Georgette</strong> – Caduta fluida</span>
              </div>
            </div>
            
            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', textAlign: 'justify', marginTop: '20px' }}>
              Per ordini all'ingrosso o informazioni, contatta il nostro team via WhatsApp o Telegram.
            </p>
          </div>
        </div>
      </section>
