...
export default function HijabsClient({ searchParams }: { searchParams?: { search?: string; page?: string; sort?: string } }) {
  ...
  const getColorCode = useCallback((color: string) => {
    const colors: any = {
      ...
      'Rose': '#e88c9a', 'Turquoise': '#40e0d0', 'Coral': '#ff7f50', 'Lavender': '#e6e6fa'
    }
  ...
  return (
    <>
      ...
      <section style={{ background: 'linear-gradient(135deg, #fdf8f2 0%, #ffffff 100%)', padding: '60px 0 40px', textAlign: 'center', borderBottom: '1px solid #f0e6dc', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', fontSize: '200px', fontWeight: 800, color: 'rgba(255,90,0,0.03)', whiteSpace: 'nowrap', zIndex: 0, pointerEvents: 'none', fontFamily: 'Playfair Display, serif' }}>HIJABS</div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', flexWrap: 'wrap', gap: '15px', background: 'white', padding: '15px 25px', borderRadius: '20px', boxShadow: '0 5px 15px rgba(0,0,0,0.03)', border: '1px solid #f0e6dc' }}>
              ...
              <div style={{ display: 'flex', gap: '15px', alignItems: 'center', flexWrap: 'wrap' }}>
                <div style={{ position: 'relative' }}>
                  <input ref={searchInputRef} type="text" placeholder="Search by name, code, fabric, color..." defaultValue={initialSearch} onChange={handleSearchChange} style={{ padding: '12px 18px 12px 42px', border: '2px solid #f0e6dc', borderRadius: '40px', fontSize: '14px', outline: 'none', width: '260px', background: 'white' }} onFocus={(e) => e.currentTarget.style.borderColor = '#ff5a00'} onBlur={(e) => e.currentTarget.style.borderColor = '#f0e6dc'} />
                ...
                {loading && products.length > 0 ? <LoadingSkeleton /> : (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '25px', marginBottom: '50px' }}>
                    {products.map((p: any) => {
                      ...
                      if (fabric.includes('modal')) fabricBadge = <span style={{ background: '#b87c4f', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>MODAL</span>
                      else if (fabric.includes('chiffon')) fabricBadge = <span style={{ background: '#d48c54', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>CHIFFON</span>
                      ...
                      else if (fabric.includes('cotton')) fabricBadge = <span style={{ background: '#c98a60', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>COTTON</span>
                      ...
                      return (
                        <Link href={getProductUrl(p)} key={p._id} style={{ background: 'white', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 5px 20px rgba(0,0,0,0.05)', transition: 'all 0.3s', textDecoration: 'none', color: 'inherit', border: '1px solid #f0e6dc', display: 'block' }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-8px)'; e.currentTarget.style.boxShadow = '0 15px 30px rgba(255,90,0,0.12)'; e.currentTarget.style.borderColor = '#ff5a00' }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 5px 20px rgba(0,0,0,0.05)'; e.currentTarget.style.borderColor = '#f0e6dc' }}>
                          <div style={{ position: 'relative', height: '280px', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #fef9f2 0%, #faf5ed 100%)' }}>
                            <img src={getProductImage(p.mainImage, p.imageUrl, { width: 400 }, p.images)} alt={p.name_en || p.name_fr || p.name_ar || 'Turkish hijab'} style={{ maxWidth: '90%', maxHeight: '90%', objectFit: 'contain', transition: 'transform 0.5s' }} loading="lazy" onError={(e) => { (e.target as HTMLImageElement).src = '/images/default.webp' }} />
                            ...
                            <div style={{ position: 'absolute', bottom: '15px', right: '15px' }}>
                              <span style={{ background: 'rgba(255,90,0,0.9)', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>{shape}</span>
                            </div>
                          </div>
                          <div style={{ padding: '20px', textAlign: 'center' }}>
                            <div style={{ fontSize: '11px', color: '#ff5a00', marginBottom: '8px', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '1px' }}>Hijab</div>
      ...
      <section style={{ background: '#fef9f2', padding: '60px 0', borderTop: '1px solid #f0e6dc', marginTop: '40px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '32px', color: '#2c2418', marginBottom: '20px', fontWeight: 700, textAlign: 'center', fontFamily: 'Playfair Display, serif' }}>Complete Turkish Hijabs Wholesale Collection</h2>
            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', marginBottom: '25px', textAlign: 'justify' }}>
              ...
              <strong>soft modal, elegant chiffon, breathable cotton, and comfortable jersey</strong>.
            </p>
            
            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>Types of Hijabs Available for Wholesale Purchase</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '35px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>Modal Hijab</strong> – Ultra-soft, lightweight, perfect drape</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>Chiffon Hijab</strong> – Elegant, lightweight, perfect for special occasions</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>Jersey Hijab</strong> – Stretchy, non-slip, comfortable one-piece</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>Cotton Hijab</strong> – Breathable, natural fabric, perfect for hot climates</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>Square Hijab</strong> – 100x100cm or 110x110cm, versatile styling</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>Rectangular Hijab</strong> – 180x70cm, easy wrap and style</span></div>
            </div>

            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>Size Chart for Turkish Hijabs</h3>
            <div style={{ background: 'white', borderRadius: '20px', padding: '30px', marginBottom: '35px', border: '1px solid #f0e6dc', boxShadow: '0 5px 20px rgba(0,0,0,0.03)' }}>
              <p style={{ textAlign: 'center', color: '#7a6a5a', marginBottom: '25px', fontSize: '14px' }}>Measurements in centimeters (cm) - Standard dimensions for Turkish hijabs</p>
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'center' }}>
                  ...
                  <tbody>
                    <tr style={{ borderBottom: '1px solid #f0e6dc' }}><td style={{ padding: '12px 10px', fontWeight: 600, color: '#ff5a00' }}>Square Standard</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>100</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>100</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>Modal, Chiffon, Cotton</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>Everyday wear</td></tr>
                    <tr style={{ borderBottom: '1px solid #f0e6dc', background: '#fef9f2' }}><td style={{ padding: '12px 10px', fontWeight: 600, color: '#ff5a00' }}>Square Large</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>110</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>110</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>Modal, Chiffon, Jersey</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>Fuller coverage</td></tr>
                    <tr style={{ borderBottom: '1px solid #f0e6dc' }}><td style={{ padding: '12px 10px', fontWeight: 600, color: '#ff5a00' }}>Rectangular Standard</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>180</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>70</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>Modal, Chiffon, Jersey</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>Versatile styling</td></tr>
                    <tr style={{ borderBottom: '1px solid #f0e6dc', background: '#fef9f2' }}><td style={{ padding: '12px 10px', fontWeight: 600, color: '#ff5a00' }}>Rectangular Long</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>200</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>80</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>Modal, Chiffon, Jersey</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>Maximum drape</td></tr>
                  </tbody>
                </table>
              </div>
              <p style={{ fontSize: '12px', color: '#a08c7a', textAlign: 'center', marginTop: '20px', fontStyle: 'italic' }}>*Measurements are approximate and may vary by manufacturer.</p>
            </div>

            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>Advantages of Turkish Hijabs for Professionals</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '35px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}>OEKO-TEX certified fabrics</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}>Wide range of colors and styles</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}>Competitive wholesale prices</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}>No minimum order quantity on most models</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}>Custom packaging and private label options</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}>Fast worldwide shipping with tracking</span></div>
            </div>
            ...
            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', textAlign: 'justify', marginTop: '20px' }}>
              ...
              Turkish textile mills produce fabrics with superior drape and breathability. Whether you need <strong>modal hijabs</strong> 
              for everyday comfort, <strong>chiffon hijabs</strong> for elegant occasions, or <strong>jersey hijabs</strong> for easy wear, 
              our wholesale collection has something for every customer.
            </p>
          </div>
        </div>
      </section>
