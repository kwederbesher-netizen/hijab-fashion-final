// app/en/category/prayer-clothes/page.tsx
'use client'
...
export default function PrayerClothesCategoryPageEn() {
  ...
  if (loading && products.length === 0) return (
    <div style={{ padding: '100px 20px', textAlign: 'center', minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      `}</style>
      <p style={{ marginTop: '20px', color: '#555', fontSize: '16px' }}>Loading prayer clothes...</p>
    </div>
  )
  ...
  return (
    <>
      <Head>
        <title>Prayer Clothes Wholesale | Turkish Prayer Dresses, Abayas for Prayer | Hijab Fashion Mall</title>
        <meta name="description" content={`Discover ${totalProducts}+ prayer clothes wholesale. Turkish prayer dresses, two-piece prayer sets, lightweight comfortable fabrics. Worldwide B2B shipping for retailers.`} />
        <meta name="keywords" content="prayer clothes wholesale, turkish prayer dress, prayer set, islamic prayer outfit, prayer abaya, two piece prayer clothes, lightweight prayer dress, breathable prayer fabric, wholesale prayer garments, muslim prayer wear" />
        <meta name="robots" content="index, follow" />
        <link rel="canonical" href="https://hijabfashionmall.com/en/category/prayer-clothes" />
        ...
        <meta property="og:title" content="Turkish Prayer Clothes Wholesale Collection" />
        <meta property="og:description" content={`${totalProducts}+ prayer garments. Comfortable, lightweight fabrics designed for convenience and ease during worship.`} />
        <meta property="og:url" content="https://hijabfashionmall.com/en/category/prayer-clothes" />
        <meta property="og:type" content="website" />
        <meta property="og:image" content="https://hijabfashionmall.com/images/og-prayer-clothes-en.jpg" />
      ...
      <section style={{ background: 'linear-gradient(135deg, #fdf8f2 0%, #ffffff 100%)', padding: '60px 0 40px', textAlign: 'center', borderBottom: '1px solid #f0e6dc', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', fontSize: '160px', fontWeight: 800, color: 'rgba(255,90,0,0.03)', whiteSpace: 'nowrap', zIndex: 0, pointerEvents: 'none', fontFamily: 'Playfair Display, serif' }}>PRAYER</div>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px', position: 'relative', zIndex: 1 }}>
          <div style={{ fontSize: '14px', color: '#7a6a5a', marginBottom: '20px' }}>
            ...
            <span style={{ color: '#7a6a5a', margin: '0 8px' }}>/</span>
            <span style={{ color: '#7a6a5a' }}>Prayer Clothes</span>
          </div>
          <h1 style={{ fontSize: '48px', color: '#2c2418', marginBottom: '15px', fontWeight: 800, lineHeight: '1.2', fontFamily: 'Playfair Display, serif' }}>
            Prayer <span style={{ color: '#ff5a00' }}>Clothes</span>
          </h1>
          <p style={{ fontSize: '18px', color: '#5a4a3a', maxWidth: '800px', margin: '0 auto', lineHeight: '1.6' }}>
            Discover our wholesale collection of Turkish prayer clothes. Comfortable, lightweight garments 
            designed for convenience and ease of wear.
          </p>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '40px', marginTop: '40px', flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>Prayer Dresses</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>Lightweight Fabrics</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>Breathable</span>
            </div>
          </div>
        </div>
      </section>
          ...
          <main>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', flexWrap: 'wrap', gap: '15px', background: 'white', padding: '15px 25px', borderRadius: '20px', boxShadow: '0 5px 15px rgba(0,0,0,0.03)', border: '1px solid #f0e6dc' }}>
              <div style={{ color: '#5a4a3a', fontSize: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span><span style={{ fontWeight: 600, color: '#ff5a00' }}>{totalProducts}</span> prayer clothes</span>
            ...
            {products.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '80px 20px', background: 'white', borderRadius: '20px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)', border: '1px solid #f0e6dc' }}>
                ...
                <h3 style={{ fontSize: '24px', color: '#2c2418', marginBottom: '10px', fontWeight: 600 }}>No prayer clothes found</h3>
                ...
                {loading && products.length > 0 ? <LoadingSkeleton /> : (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '25px', marginBottom: '50px' }}>
                    {products.map((p: any) => {
                      ...
                      if (fabric.includes('cotton')) fabricBadge = <span style={{ background: '#2d6a4f', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>COTTON</span>
                      ...
                      else if (fabric.includes('modal')) fabricBadge = <span style={{ background: '#b87c4f', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>MODAL</span>
                      ...
                      return (
                        <Link href={getProductUrl(p)} key={p._id} style={{ background: 'white', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 5px 20px rgba(0,0,0,0.05)', transition: 'all 0.3s', textDecoration: 'none', color: 'inherit', border: '1px solid #f0e6dc', display: 'block' }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-8px)'; e.currentTarget.style.boxShadow = '0 15px 30px rgba(255,90,0,0.12)'; e.currentTarget.style.borderColor = '#ff5a00' }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 5px 20px rgba(0,0,0,0.05)'; e.currentTarget.style.borderColor = '#f0e6dc' }}>
                          <div style={{ position: 'relative', height: '280px', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #fef9f2 0%, #faf5ed 100%)' }}>
                            <img src={p.imageUrl || '/images/default.webp'} alt={p.name_en || p.name_fr || p.name_ar || 'Prayer clothes'} style={{ maxWidth: '90%', maxHeight: '90%', objectFit: 'contain', transition: 'transform 0.5s' }} loading="lazy" onError={(e) => { (e.target as HTMLImageElement).src = '/images/default.webp' }} />
                          ...
                          <div style={{ padding: '20px', textAlign: 'center' }}>
                            <div style={{ fontSize: '11px', color: '#ff5a00', marginBottom: '8px', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '1px' }}>Prayer Garment</div>
      ...
      <section style={{ background: '#fef9f2', padding: '60px 0', borderTop: '1px solid #f0e6dc', marginTop: '40px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '32px', color: '#2c2418', marginBottom: '20px', fontWeight: 700, textAlign: 'center', fontFamily: 'Playfair Display, serif' }}>Turkish Prayer Clothes Collection</h2>
            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', marginBottom: '25px', textAlign: 'justify' }}>
              Explore our collection of <strong>Turkish prayer clothes</strong>, designed for convenience and comfort. 
              Our wholesale range features over <strong>{totalProducts} prayer garments</strong> including one-piece prayer dresses 
              and two-piece sets. These garments are made from lightweight, breathable fabrics that are easy to care for 
              and comfortable to wear. The simple, practical designs focus on ease of use while meeting coverage requirements.
            </p>
            
            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>Prayer Garment Styles Available Wholesale</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '35px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>One-Piece Prayer Dresses</strong> – All-in-one garment, easy to wear</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>Two-Piece Prayer Sets</strong> – Top and skirt combination</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>Prayer Abayas</strong> – Open front or closed styles</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>Prayer Hijabs</strong> – Matching head coverings</span></div>
            </div>

            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>Size Chart for Prayer Clothes</h3>
            <div style={{ background: 'white', borderRadius: '20px', padding: '30px', marginBottom: '35px', border: '1px solid #f0e6dc', boxShadow: '0 5px 20px rgba(0,0,0,0.03)' }}>
              <p style={{ textAlign: 'center', color: '#7a6a5a', marginBottom: '25px', fontSize: '14px' }}>Measurements in centimeters (cm) - Relaxed fit for comfort</p>
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'center' }}>
                  <thead><tr style={{ background: '#fef9f2', borderBottom: '2px solid #ff5a00' }}><th style={{ padding: '15px 10px', fontWeight: 700, color: '#2c2418' }}>Size</th><th style={{ padding: '15px 10px', fontWeight: 700, color: '#2c2418' }}>Bust (cm)</th><th style={{ padding: '15px 10px', fontWeight: 700, color: '#2c2418' }}>Waist (cm)</th><th style={{ padding: '15px 10px', fontWeight: 700, color: '#2c2418' }}>Hip (cm)</th><th style={{ padding: '15px 10px', fontWeight: 700, color: '#2c2418' }}>Garment Length (cm)</th><th style={{ padding: '15px 10px', fontWeight: 700, color: '#2c2418' }}>US Size</th></tr></thead>
                  <tbody>
                    <tr style={{ borderBottom: '1px solid #f0e6dc' }}><td style={{ padding: '12px 10px', fontWeight: 600, color: '#ff5a00' }}>S/M</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>86-96</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>66-76</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>92-102</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>135-145</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>4-10</td></tr>
                    <tr style={{ borderBottom: '1px solid #f0e6dc', background: '#fef9f2' }}><td style={{ padding: '12px 10px', fontWeight: 600, color: '#ff5a00' }}>L/XL</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>96-106</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>76-86</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>102-112</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>140-150</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>12-18</td></tr>
                    <tr style={{ borderBottom: '1px solid #f0e6dc' }}><td style={{ padding: '12px 10px', fontWeight: 600, color: '#ff5a00' }}>2XL/3XL</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>106-118</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>86-98</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>112-122</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>145-155</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>20-26</td></tr>
                    <tr style={{ borderBottom: '1px solid #f0e6dc', background: '#fef9f2' }}><td style={{ padding: '12px 10px', fontWeight: 600, color: '#ff5a00' }}>4XL</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>118-124</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>98-104</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>122-128</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>150-155</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>28-32</td></tr>
                  </tbody>
                </table>
              </div>
              <p style={{ fontSize: '12px', color: '#a08c7a', textAlign: 'center', marginTop: '20px', fontStyle: 'italic' }}>*Prayer clothes are designed with a relaxed, comfortable fit. Most styles are one-size or size-inclusive.</p>
            </div>

            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>Key Features of Turkish Prayer Clothes</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '35px' }}>
              <div style={{ padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><span style={{ fontWeight: 700, color: '#ff5a00' }}>Lightweight:</span><p style={{ color: '#4a3a2a', marginTop: '5px', fontSize: '14px' }}>Easy to fold and carry for travel</p></div>
              <div style={{ padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><span style={{ fontWeight: 700, color: '#ff5a00' }}>Breathable:</span><p style={{ color: '#4a3a2a', marginTop: '5px', fontSize: '14px' }}>Cotton, jersey, and modal fabrics for comfort</p></div>
              <div style={{ padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><span style={{ fontWeight: 700, color: '#ff5a00' }}>Easy Care:</span><p style={{ color: '#4a3a2a', marginTop: '5px', fontSize: '14px' }}>Machine washable, quick drying</p></div>
              <div style={{ padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><span style={{ fontWeight: 700, color: '#ff5a00' }}>Full Coverage:</span><p style={{ color: '#4a3a2a', marginTop: '5px', fontSize: '14px' }}>Designed to meet coverage requirements</p></div>
              <div style={{ padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><span style={{ fontWeight: 700, color: '#ff5a00' }}>No Ironing:</span><p style={{ color: '#4a3a2a', marginTop: '5px', fontSize: '14px' }}>Wrinkle-resistant fabrics on select styles</p></div>
              <div style={{ padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><span style={{ fontWeight: 700, color: '#ff5a00' }}>Neutral Colors:</span><p style={{ color: '#4a3a2a', marginTop: '5px', fontSize: '14px' }}>White, cream, beige, pastels, and dark tones</p></div>
            </div>

            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', textAlign: 'justify', marginTop: '20px' }}>
              Turkish prayer clothes offer practical, comfortable options for daily use. These garments are designed with 
              attention to fabric quality and ease of care, making them suitable for regular use. The simple, functional 
              designs focus on convenience while meeting coverage needs. Our wholesale collection includes various styles 
              to suit different preferences. Browse our prayer clothes selection and offer your customers comfortable, 
              practical options.
            </p>
          </div>
        </div>
