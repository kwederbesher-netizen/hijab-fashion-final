      ...
      <section style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px 20px 0' }}>
        <div style={{ background: '#fef9f2', borderRadius: '24px', padding: '20px 25px', border: '1px solid #f0e6dc' }}>
          <h3 style={{ fontSize: '18px', color: '#2c2418', marginBottom: '15px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            Größentabelle - Hijabs
          </h3>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
              <thead>
                <tr style={{ background: '#fff0e6', borderBottom: '2px solid #ff5a00' }}>
                  <th style={{ padding: '12px', textAlign: 'center', fontWeight: 600, color: '#2c2418' }}>Typ</th>
                  <th style={{ padding: '12px', textAlign: 'center', fontWeight: 600, color: '#2c2418' }}>Größe (cm)</th>
                  <th style={{ padding: '12px', textAlign: 'center', fontWeight: 600, color: '#2c2418' }}>Material</th>
                  <th style={{ padding: '12px', textAlign: 'center', fontWeight: 600, color: '#2c2418' }}>Besonderheit</th>
                </tr>
              </thead>
              <tbody>
                <tr style={{ borderBottom: '1px solid #f0e6dc' }}><td style={{ padding: '10px', textAlign: 'center', fontWeight: 600, color: '#ff5a00' }}>Square</td><td style={{ padding: '10px', textAlign: 'center', color: '#5a4a3a' }}>110x110 / 115x115</td><td style={{ padding: '10px', textAlign: 'center', color: '#5a4a3a' }}>Viskose, Jersey, Bambus</td><td style={{ padding: '10px', textAlign: 'center', color: '#5a4a3a' }}>Klassisch</td></tr>
                <tr style={{ borderBottom: '1px solid #f0e6dc' }}><td style={{ padding: '10px', textAlign: 'center', fontWeight: 600, color: '#ff5a00' }}>Instant</td><td style={{ padding: '10px', textAlign: 'center', color: '#5a4a3a' }}>Einheitsgröße</td><td style={{ padding: '10px', textAlign: 'center', color: '#5a4a3a' }}>Jersey, Modal</td><td style={{ padding: '10px', textAlign: 'center', color: '#5a4a3a' }}>Mit/ohne Draht</td></tr>
                <tr style={{ borderBottom: '1px solid #f0e6dc' }}><td style={{ padding: '10px', textAlign: 'center', fontWeight: 600, color: '#ff5a00' }}>Chiffon</td><td style={{ padding: '10px', textAlign: 'center', color: '#5a4a3a' }}>110x110 / 115x115</td><td style={{ padding: '10px', textAlign: 'center', color: '#5a4a3a' }}>Chiffon</td><td style={{ padding: '10px', textAlign: 'center', color: '#5a4a3a' }}>Leicht, transparent</td></tr>
                <tr style={{ borderBottom: '1px solid #f0e6dc' }}><td style={{ padding: '10px', textAlign: 'center', fontWeight: 600, color: '#ff5a00' }}>Seide</td><td style={{ padding: '10px', textAlign: 'center', color: '#5a4a3a' }}>110x110 / 115x115</td><td style={{ padding: '10px', textAlign: 'center', color: '#5a4a3a' }}>Seide / Satin</td><td style={{ padding: '10px', textAlign: 'center', color: '#5a4a3a' }}>Luxuriös</td></tr>
              </tbody>
            </table>
          </div>
          <p style={{ fontSize: '12px', color: '#a08c7a', marginTop: '12px', textAlign: 'center' }}>Alle Hijabs sind Einheitsgröße und passen für die meisten Kopfgrößen.</p>
        </div>
      </section>
                ...
                {loading && products.length > 0 ? <LoadingSkeleton /> : (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '25px', marginBottom: '50px' }}>
                    {products.map((p: any) => {
                      ...
                      if (fabric.includes('bamboo')) fabricBadge = <span style={{ background: '#b87c4f', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>BAMBUS</span>
                      ...
                      else if (fabric.includes('silk')) fabricBadge = <span style={{ background: '#c98a60', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>SEIDE</span>
                      else if (fabric.includes('chiffon')) fabricBadge = <span style={{ background: '#e8a87c', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>CHIFFON</span>
                      else if (fabric.includes('modal')) fabricBadge = <span style={{ background: '#b5a07e', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>MODAL</span>
                      ...
                      return (
                        <Link href={getProductUrl(p)} key={p._id} style={{ background: 'white', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 5px 20px rgba(0,0,0,0.05)', transition: 'all 0.3s', textDecoration: 'none', color: 'inherit', border: '1px solid #f0e6dc', display: 'block' }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-8px)'; e.currentTarget.style.boxShadow = '0 15px 30px rgba(255,90,0,0.12)'; e.currentTarget.style.borderColor = '#ff5a00'; }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 5px 20px rgba(0,0,0,0.05)'; e.currentTarget.style.borderColor = '#f0e6dc'; }}>
                          <div style={{ position: 'relative', height: '280px', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #fef9f2 0%, #faf5ed 100%)' }}>
                            <img src={getProductImage(p.mainImage, p.imageUrl, { width: 400 }, p.images)} alt={p.name_de || p.name_en || p.name_ar || 'Türkischer Hijab'} style={{ maxWidth: '90%', maxHeight: '90%', objectFit: 'contain', transition: 'transform 0.5s' }} loading="lazy" onError={(e) => { (e.target as HTMLImageElement).src = '/images/default.webp' }} />
      ...
      <section style={{ background: '#fef9f2', padding: '60px 0', borderTop: '1px solid #f0e6dc', marginTop: '40px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '32px', color: '#2c2418', marginBottom: '20px', fontWeight: 700, textAlign: 'center', fontFamily: 'Playfair Display, serif' }}>Vollständige Kollektion Türkischer Hijabs im Großhandel</h2>
            ...
            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>Verfügbare Hijab-Typen im Großhandel</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '35px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5"/></svg><span style={{ color: '#4a3a2a' }}><strong>Bambus-Hijab</strong> – Besonders weich, hautfreundlich</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5"/></svg><span style={{ color: '#4a3a2a' }}><strong>Viskose-Hijab</strong> – Fließender Fall, leicht</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5"/></svg><span style={{ color: '#4a3a2a' }}><strong>Jersey-Hijab</strong> – Dehnbar, knitterfrei</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5"/></svg><span style={{ color: '#4a3a2a' }}><strong>Seiden-Hijab</strong> – Luxuriöser Glanz</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5"/></svg><span style={{ color: '#4a3a2a' }}><strong>Chiffon-Hijab</strong> – Leicht, transparent, modern</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5"/></svg><span style={{ color: '#4a3a2a' }}><strong>Hijab mit Draht</strong> – Eingearbeiteter Draht für perfekte Form</span></div>
            </div>
            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', textAlign: 'justify' }}>
              <strong>Türkische Hijabs</strong> sind international bekannt für ihre hohe Verarbeitungsqualität und das moderne Farbangebot.
              Für Großhandelsbestellungen, Musteranfragen oder weitere Informationen kontaktieren Sie unser Team.
            </p>
          </div>
        </div>
      </section>
