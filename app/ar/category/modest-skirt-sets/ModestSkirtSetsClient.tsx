...
export default function ModestSkirtSetsClient({ searchParams }: { searchParams?: { search?: string; page?: string; sort?: string } }) {
  ...
  return (
    <>
          ...
          <main>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', flexWrap: 'wrap', gap: '15px', background: 'white', padding: '15px 25px', borderRadius: '20px', boxShadow: '0 5px 15px rgba(0,0,0,0.03)', border: '1px solid #f0e6dc' }}>
              ...
              <div style={{ display: 'flex', gap: '15px', alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                <select value={sortBy} onChange={(e) => handleSortChange(e.target.value)} style={{ padding: '12px 20px', border: '2px solid #f0e6dc', borderRadius: '40px', fontFamily: 'Poppins, sans-serif', fontSize: '14px', cursor: 'pointer', outline: 'none', background: 'white', minWidth: '200px', color: '#2c2418' }}>
                ...
                <button
                  ...
                  style={{ padding: '12px 20px', border: '2px solid #f0e6dc', borderRadius: '40px', background: 'white', cursor: 'pointer', fontSize: '14px', fontWeight: 500, color: '#2c2418', transition: 'all 0.3s', display: 'flex', alignItems: 'center', gap: '8px' }}
      ...
      {/* Size Chart Modal */}
      {showSizeChart && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }} onClick={() => setShowSizeChart(false)}>
          <div style={{ background: 'white', borderRadius: '20px', maxWidth: '900px', width: '100%', maxHeight: '80vh', overflow: 'auto', padding: '30px', position: 'relative' }} onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setShowSizeChart(false)} style={{ position: 'absolute', top: '15px', left: '15px', background: 'none', border: 'none', fontSize: '24px', cursor: 'pointer', color: '#666' }}>✕</button>
            <h2 style={{ fontSize: '24px', color: '#2c2418', marginBottom: '20px', textAlign: 'center' }}>دليل مقاسات التنانير والبلوزات التركية</h2>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#ff5a00', color: 'white' }}>
                    ...
                    <th style={{ padding: '12px', border: '1px solid #ddd' }}>الخصر (سم)</th>
                    <th style={{ padding: '12px', border: '1px solid #ddd' }}>الورك (سم)</th>
                    <th style={{ padding: '12px', border: '1px solid #ddd' }}>طول التنورة (سم)</th>
                    <th style={{ padding: '12px', border: '1px solid #ddd' }}>طول البلوزة (سم)</th>
                  </tr>
                </thead>
                <tbody>
                  {[
                    ...
                    <tr key={row.size} style={{ background: idx % 2 === 0 ? '#fef9f2' : 'white' }}>
                      <td style={{ padding: '10px', border: '1px solid #ddd', fontWeight: 600, color: '#ff5a00' }}>{row.size}</td>
                      <td style={{ padding: '10px', border: '1px solid #ddd' }}>{row.chest}</td>
                      <td style={{ padding: '10px', border: '1px solid #ddd' }}>{row.waist}</td>
                      <td style={{ padding: '10px', border: '1px solid #ddd' }}>{row.hip}</td>
                      <td style={{ padding: '10px', border: '1px solid #ddd' }}>{row.skirtLen}</td>
                      <td style={{ padding: '10px', border: '1px solid #ddd' }}>{row.topLen}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            <div style={{ marginTop: '20px', padding: '15px', background: '#fef9f2', borderRadius: '12px' }}>
              <h3 style={{ fontSize: '16px', marginBottom: '10px' }}>نصائح لاختيار المقاس المناسب:</h3>
