...
export default function ModestSkirtSetsClient({ searchParams }: { searchParams?: { search?: string; page?: string; sort?: string } }) {
  ...
  if (loading && products.length === 0) return (
    <div style={{ padding: '100px 20px', textAlign: 'center', minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      `}</style>
      <p style={{ marginTop: '20px', color: '#555', fontSize: '16px' }}>جاري تحميل أطقم التنانير...</p>
    </div>
  )
  ...
  return (
    <>
      ...
      <section style={{ background: 'linear-gradient(135deg, #fdf8f2 0%, #ffffff 100%)', padding: '60px 0 40px', textAlign: 'center', borderBottom: '1px solid #f0e6dc', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', fontSize: '160px', fontWeight: 800, color: 'rgba(255,90,0,0.03)', whiteSpace: 'nowrap', zIndex: 0, pointerEvents: 'none', fontFamily: 'Playfair Display, serif' }}>أطقم تنانير</div>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px', position: 'relative', zIndex: 1 }}>
          <div style={{ fontSize: '14px', color: '#7a6a5a', marginBottom: '20px' }}>
            ...
            <span style={{ color: '#7a6a5a', margin: '0 8px' }}>/</span>
            <span style={{ color: '#7a6a5a' }}>أطقم تنانير محجبات</span>
          </div>
          <h1 style={{ fontSize: '48px', color: '#2c2418', marginBottom: '15px', fontWeight: 800, lineHeight: '1.2', fontFamily: 'Playfair Display, serif' }}>
            أطقم <span style={{ color: '#ff5a00' }}>تنانير محجبات</span>
          </h1>
          <p style={{ fontSize: '18px', color: '#5a4a3a', maxWidth: '800px', margin: '0 auto', lineHeight: '1.6' }}>
            اكتشفي مجموعتنا بالجملة من أطقم التنانير التركية للمحجبات. تصاميم من قطعتين متناسقتين 
            تجمع بين التنانير الطويلة والبلوزات المتطابقة لإطلالة أنيقة ومريحة.
          </p>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '40px', marginTop: '40px', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>بلوزات متناسقة</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <main>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', flexWrap: 'wrap', gap: '15px', background: 'white', padding: '15px 25px', borderRadius: '20px', boxShadow: '0 5px 15px rgba(0,0,0,0.03)', border: '1px solid #f0e6dc' }}>
              <div style={{ color: '#5a4a3a', fontSize: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span><span style={{ fontWeight: 600, color: '#ff5a00' }}>{totalProducts}</span> طقم تنورة</span>
            ...
            {products.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '80px 20px', background: 'white', borderRadius: '20px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)', border: '1px solid #f0e6dc' }}>
                ...
                <h3 style={{ fontSize: '24px', color: '#2c2418', marginBottom: '10px', fontWeight: 600 }}>لا توجد أطقم تنانير</h3>
                ...
                {loading && products.length > 0 ? <LoadingSkeleton /> : (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '25px', marginBottom: '50px' }}>
                    {products.map((p: any) => {
                      ...
                      if (fabric.includes('شيفون')) fabricBadge = <span style={{ background: '#d48c54', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>شيفون</span>
                      else if (fabric.includes('كريب')) fabricBadge = <span style={{ background: '#b87c4f', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>كريب</span>
                      ...
                      return (
                        <Link href={getProductUrl(p)} key={p._id} style={{ background: 'white', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 5px 20px rgba(0,0,0,0.05)', transition: 'all 0.3s', textDecoration: 'none', color: 'inherit', border: '1px solid #f0e6dc', display: 'block' }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-8px)'; e.currentTarget.style.boxShadow = '0 15px 30px rgba(255,90,0,0.12)'; e.currentTarget.style.borderColor = '#ff5a00' }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 5px 20px rgba(0,0,0,0.05)'; e.currentTarget.style.borderColor = '#f0e6dc' }}>
                          <div style={{ position: 'relative', height: '280px', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #fef9f2 0%, #faf5ed 100%)' }}>
                            <img src={getProductImage(p.mainImage, p.imageUrl, { width: 400 }, p.images)} alt={p.name_ar || p.name_en || p.name_fr || 'طقم تنورة'} style={{ maxWidth: '90%', maxHeight: '90%', objectFit: 'contain', transition: 'transform 0.5s' }} loading="lazy" onError={(e) => { (e.target as HTMLImageElement).src = '/images/default.webp' }} />
                          ...
                          <div style={{ padding: '20px', textAlign: 'center' }}>
                            <div style={{ fontSize: '11px', color: '#ff5a00', marginBottom: '8px', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '1px' }}>طقم تنورة</div>
      ...
      <section style={{ background: '#fef9f2', padding: '60px 0', borderTop: '1px solid #f0e6dc', marginTop: '40px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '32px', color: '#2c2418', marginBottom: '20px', fontWeight: 700, textAlign: 'center', fontFamily: 'Playfair Display, serif' }}>مجموعة أطقم التنانير التركية للمحجبات</h2>
            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', marginBottom: '25px', textAlign: 'justify' }}>
              اكتشفي مجموعتنا من <strong>أطقم التنانير التركية للمحجبات</strong>، تصاميم متناسقة من قطعتين مصممة للنساء 
              اللواتي يقدرن الأناقة السهلة. تشمل مجموعتنا بالجملة أكثر من <strong>{totalProducts} موديل طقم تنورة</strong> 
              تجمع بين التنانير الطويلة الانسيابية والبلوزات المتناسقة بأقمشة فاخرة.
            </p>
            
            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>أنواع أطقم التنانير المتاحة للشراء بالجملة</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '35px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>أطقم تنانير ماكسي</strong> – تنانير بطول الكاحل مع بلوزات طويلة متناسقة</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>أطقم تنانير مكشكشة</strong> – طيات أكورديون لحركة أنيقة</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>أطقم تنانير A-line</strong> – قصة كلاسيكية، مقاس مناسب</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>أطقم تنانير مستقيمة</strong> – مظهر أنيق واحترافي للعمل</span></div>
            </div>

            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>جدول المقاسات لأطقم التنانير التركية</h3>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'center' }}>
                  <thead>
                    <tr style={{ background: '#fef9f2', borderBottom: '2px solid #ff5a00' }}>
                      ...
                      <th style={{ padding: '15px 10px', fontWeight: 700, color: '#2c2418' }}>طول التنورة (سم)</th>
  ...
  <td style={{ padding: '12px 10px', color: '#5a4a3a' }}>85-90</td>
...
<p style={{ fontSize: '12px', color: '#a08c7a', textAlign: 'center', marginTop: '20px', fontStyle: 'italic' }}>*المقاسات تقريبية. قد تختلف أطوال التنانير حسب الموديل.</p>
</div>

<p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', textAlign: 'justify', marginTop: '20px' }}>
تقدم أطقم التنانير التركية للمحجبات حلاً كاملاً للملابس لتجار التجزئة الذين يخدمون العملاء الذين يقدرون الراحة والتناسق. 
تتميز هذه الأطقم المكونة من قطعتين <strong>بتنانير طويلة مع بلوزات متطابقة</strong> بألوان وأقمشة متكاملة. 
يمكن ارتداء القطع المنفصلة معاً كطقم أو مزجها مع عناصر خزانة الملابس الموجودة.
</p>
</div>
</div>
</section>
