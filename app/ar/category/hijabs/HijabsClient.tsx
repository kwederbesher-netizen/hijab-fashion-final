      ...
      <section style={{ background: 'linear-gradient(135deg, #fdf8f2 0%, #ffffff 100%)', padding: '60px 0 40px', textAlign: 'center', borderBottom: '1px solid #f0e6dc', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px', position: 'relative', zIndex: 1 }}>
          <div style={{ fontSize: '14px', color: '#7a6a5a', marginBottom: '20px' }}>
            ...
            <span style={{ color: '#7a6a5a', margin: '0 8px' }}>/</span>
            <span style={{ color: '#7a6a5a' }}>حجابات</span>
          </div>
          <h1 style={{ fontSize: '48px', color: '#2c2418', marginBottom: '15px', fontWeight: 800, lineHeight: '1.2', fontFamily: 'Playfair Display, serif' }}>
            حجابات تركية <span style={{ color: '#ff5a00' }}>فاخرة</span>
          </h1>
          <p style={{ fontSize: '18px', color: '#5a4a3a', maxWidth: '800px', margin: '0 auto', lineHeight: '1.6' }}>
            اكتشفي مجموعتنا بالجملة من الحجابات التركية. مودال ناعم، شيفون أنيق، 
            قطن قابل للتنفس، وجيرسي مريح في تشكيلة واسعة من الألوان والأنماط.
          </p>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '40px', marginTop: '40px', flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>مودال فاخر</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>شيفون وجيرسي</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>أكثر من 100 لون</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>مربعة ومستطيلة</span>
            </div>
          </div>
        </div>
      </section>
          ...
          <main>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', flexWrap: 'wrap', gap: '15px', background: 'white', padding: '15px 25px', borderRadius: '20px', boxShadow: '0 5px 15px rgba(0,0,0,0.03)', border: '1px solid #f0e6dc' }}>
              <div style={{ color: '#5a4a3a', fontSize: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span><span style={{ fontWeight: 600, color: '#ff5a00' }}>{totalProducts}</span> حجاب مرجعي</span>
                {totalProducts > 0 && <span style={{ marginLeft: '10px', color: '#a08c7a', fontSize: '13px' }}>({startIndex}-{endIndex})</span>}
              </div>
              <div style={{ display: 'flex', gap: '15px', alignItems: 'center', flexWrap: 'wrap' }}>
                <div style={{ position: 'relative' }}>
                  <input ref={searchInputRef} type="text" placeholder="ابحث بالاسم، الرمز، القماش، اللون..." defaultValue={initialSearch} onChange={handleSearchChange} style={{ padding: '12px 18px 12px 42px', border: '2px solid #f0e6dc', borderRadius: '40px', fontSize: '14px', outline: 'none', width: '260px', background: 'white' }} onFocus={(e) => e.currentTarget.style.borderColor = '#ff5a00'} onBlur={(e) => e.currentTarget.style.borderColor = '#f0e6dc'} />
            ...
            {products.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '80px 20px', background: 'white', borderRadius: '20px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)', border: '1px solid #f0e6dc' }}>
                ...
                <h3 style={{ fontSize: '24px', color: '#2c2418', marginBottom: '10px', fontWeight: 600 }}>لا توجد حجابات</h3>
                ...
                {loading && products.length > 0 ? <LoadingSkeleton /> : (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '25px', marginBottom: '50px' }}>
                    {products.map((p: any) => {
                      ...
                      if (fabric.includes('مودال')) fabricBadge = <span style={{ background: '#b87c4f', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>مودال</span>
                      else if (fabric.includes('شيفون')) fabricBadge = <span style={{ background: '#d48c54', color: 'white', padding: '4px 12px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>شيفون</span>
                      ...
                      const isSquare = (p.name_ar || p.name_en || '').toLowerCase().includes('مربع')
                      const shape = isSquare ? 'مربعة' : 'مستطيلة'
                      ...
                      return (
                        <Link href={getProductUrl(p)} key={p._id} style={{ background: 'white', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 5px 20px rgba(0,0,0,0.05)', transition: 'all 0.3s', textDecoration: 'none', color: 'inherit', border: '1px solid #f0e6dc', display: 'block' }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-8px)'; e.currentTarget.style.boxShadow = '0 15px 30px rgba(255,90,0,0.12)'; e.currentTarget.style.borderColor = '#ff5a00' }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 5px 20px rgba(0,0,0,0.05)'; e.currentTarget.style.borderColor = '#f0e6dc' }}>
                          <div style={{ position: 'relative', height: '280px', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #fef9f2 0%, #faf5ed 100%)' }}>
                            <img src={getProductImage(p.mainImage, p.imageUrl, { width: 400 }, p.images)} alt={p.name_ar || p.name_en || p.name_fr || 'حجاب تركي'} style={{ maxWidth: '90%', maxHeight: '90%', objectFit: 'contain', transition: 'transform 0.5s' }} loading="lazy" onError={(e) => { (e.target as HTMLImageElement).src = '/images/default.webp' }} />
                          ...
                          <div style={{ padding: '20px', textAlign: 'center' }}>
                            ...
                            {p.color_ar || p.color_en || p.color_fr ? (
                              <div style={{ marginBottom: '12px' }}>
                                ...
                                <div style={{ fontSize: '11px', color: '#a08c7a', marginTop: '5px' }}>{p.color_ar || p.color_en || p.color_fr}</div>
                              </div>
                            ) : null}
      ...
      <section style={{ background: '#fef9f2', padding: '60px 0', borderTop: '1px solid #f0e6dc', marginTop: '40px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '32px', color: '#2c2418', marginBottom: '20px', fontWeight: 700, textAlign: 'center', fontFamily: 'Playfair Display, serif' }}>مجموعة كاملة من الحجابات التركية بالجملة</h2>
            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', marginBottom: '25px', textAlign: 'justify' }}>
              مرحباً بكم في حجاب فاشون مول، <strong>متجركم الموثوق للحجابات التركية بالجملة</strong> منذ 2019. 
              تضم مجموعتنا بالجملة أكثر من <strong>{totalProducts} موديل حجاب</strong> مصممة لتلبية احتياجات 
              تجار التجزئة والمتاجر الإلكترونية. نقدم أقمشة عالية الجودة: 
              <strong>مودال ناعم، شيفون أنيق، قطن قابل للتنفس، وجيرسي مريح</strong>.
            </p>
            
            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>أنواع الحجابات المتاحة للشراء بالجملة</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '35px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>حجاب مودال</strong> – ناعم جداً، خفيف الوزن، انسيابية مثالية</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>حجاب شيفون</strong> – أنيق، خفيف الوزن، مثالي للمناسبات الخاصة</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>حجاب جيرسي</strong> – مطاطي، غير قابل للانزلاق، قطعة واحدة مريحة</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>حجاب قطن</strong> – قابل للتنفس، قماش طبيعي، مثالي للمناخ الحار</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>حجاب مربع</strong> – 100×100 سم أو 110×110 سم، تسريحات متعددة</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>حجاب مستطيل</strong> – 180×70 سم، لف سهل وأنيق</span></div>
            </div>

            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>جدول المقاسات للحجابات التركية</h3>
            <div style={{ background: 'white', borderRadius: '20px', padding: '30px', marginBottom: '35px', border: '1px solid #f0e6dc', boxShadow: '0 5px 20px rgba(0,0,0,0.03)' }}>
              <p style={{ textAlign: 'center', color: '#7a6a5a', marginBottom: '25px', fontSize: '14px' }}>المقاسات بالسنتيمتر (سم) - الأبعاد القياسية للحجابات التركية</p>
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'center' }}>
                  <thead>
                    <tr style={{ background: '#fef9f2', borderBottom: '2px solid #ff5a00' }}>
                      <th style={{ padding: '15px 10px', fontWeight: 700, color: '#2c2418' }}>النوع</th>
                      <th style={{ padding: '15px 10px', fontWeight: 700, color: '#2c2418' }}>الطول (سم)</th>
                      <th style={{ padding: '15px 10px', fontWeight: 700, color: '#2c2418' }}>العرض (سم)</th>
                      <th style={{ padding: '15px 10px', fontWeight: 700, color: '#2c2418' }}>أنواع الأقمشة</th>
                      <th style={{ padding: '15px 10px', fontWeight: 700, color: '#2c2418' }}>الاستخدام المناسب</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr style={{ borderBottom: '1px solid #f0e6dc' }}><td style={{ padding: '12px 10px', fontWeight: 600, color: '#ff5a00' }}>مربع قياسي</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>100</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>100</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>مودال، شيفون، قطن</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>الاستخدام اليومي</td></tr>
                    <tr style={{ borderBottom: '1px solid #f0e6dc', background: '#fef9f2' }}><td style={{ padding: '12px 10px', fontWeight: 600, color: '#ff5a00' }}>مربع كبير</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>110</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>110</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>مودال، شيفون، جيرسي</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>تغطية أكبر</td></tr>
                    <tr style={{ borderBottom: '1px solid #f0e6dc' }}><td style={{ padding: '12px 10px', fontWeight: 600, color: '#ff5a00' }}>مستطيل قياسي</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>180</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>70</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>مودال، شيفون، جيرسي</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>تسريحات متعددة</td></tr>
                    <tr style={{ borderBottom: '1px solid #f0e6dc', background: '#fef9f2' }}><td style={{ padding: '12px 10px', fontWeight: 600, color: '#ff5a00' }}>مستطيل طويل</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>200</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>80</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>مودال، شيفون، جيرسي</td><td style={{ padding: '12px 10px', color: '#5a4a3a' }}>أقصى انسيابية</td></tr>
                  </tbody>
                </table>
              </div>
              <p style={{ fontSize: '12px', color: '#a08c7a', textAlign: 'center', marginTop: '20px', fontStyle: 'italic' }}>*المقاسات تقريبية وقد تختلف حسب الشركة المصنعة.</p>
            </div>

            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>مزايا الحجابات التركية للمحترفين</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '35px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}>أقمشة معتمدة عالية الجودة</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}>تشكيلة واسعة من الألوان والأنماط</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}>أسعار تنافسية بالجملة</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}>لا يوجد حد أدنى للطلب على معظم الموديلات</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}>تغليف مخصص وعلامة خاصة</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}>شحن سريع مع تتبع</span></div>
            </div>

            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', textAlign: 'justify', marginTop: '20px' }}>
              <strong>الحجابات التركية</strong> مشهورة عالمياً بجودتها الاستثنائية ونعومتها ومتانتها. 
              تنتج مصانع النسيج التركية أقمشة ذات انسيابية وتهوية فائقة. سواء كنت بحاجة <strong>حجابات مودال</strong> 
              للراحة اليومية، أو <strong>حجابات شيفون</strong> للمناسبات الأنيقة، أو <strong>حجابات جيرسي</strong> للارتداء السهل، 
              فإن مجموعتنا بالجملة لديها ما يناسب كل عميل.
            </p>
          </div>
        </div>
      </section>
