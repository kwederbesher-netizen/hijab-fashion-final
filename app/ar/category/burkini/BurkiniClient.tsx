...
export default function BurkiniClient({ searchParams }: { searchParams?: { search?: string; page?: string; sort?: string } }) {
  ...
  if (loading && products.length === 0) return (
    <div style={{ padding: '100px 20px', textAlign: 'center', minHeight: '60vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      `}</style>
      <p style={{ marginTop: '20px', color: '#555', fontSize: '16px' }}>جاري تحميل البوركيني...</p>
    </div>
  )
  ...
  return (
    <>
      ...
      <section style={{ background: 'linear-gradient(135deg, #fdf8f2 0%, #ffffff 100%)', padding: '60px 0 40px', textAlign: 'center', borderBottom: '1px solid #f0e6dc', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', fontSize: '160px', fontWeight: 800, color: 'rgba(255,90,0,0.03)', whiteSpace: 'nowrap', zIndex: 0, pointerEvents: 'none', fontFamily: 'Playfair Display, serif' }}>بوركيني</div>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px', position: 'relative', zIndex: 1 }}>
          <div style={{ fontSize: '14px', color: '#7a6a5a', marginBottom: '20px' }}>
            ...
            <span style={{ color: '#7a6a5a', margin: '0 8px' }}>/</span>
            <span style={{ color: '#7a6a5a' }}>بوركيني وملابس سباحة</span>
          </div>
          <h1 style={{ fontSize: '48px', color: '#2c2418', marginBottom: '15px', fontWeight: 800, lineHeight: '1.2', fontFamily: 'Playfair Display, serif' }}>
            بوركيني <span style={{ color: '#ff5a00' }}>محجبات</span>
          </h1>
          <p style={{ fontSize: '18px', color: '#5a4a3a', maxWidth: '800px', margin: '0 auto', lineHeight: '1.6' }}>
            اكتشفي مجموعتنا بالجملة من البوركيني التركي. ملابس سباحة بتغطية كاملة مصممة 
            للراحة والجفاف السريع وحرية الحركة في الماء.
          </p>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '40px', marginTop: '40px', flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>طقم من قطعتين</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>جفاف سريع</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>حماية من الأشعة فوق البنفسجية</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,90,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <span style={{ color: '#4a3a2a', fontSize: '15px' }}>حجاب سباحة</span>
            </div>
          </div>
        </div>
      </section>
          ...
          <main>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', flexWrap: 'wrap', gap: '15px', background: 'white', padding: '15px 25px', borderRadius: '20px', boxShadow: '0 5px 15px rgba(0,0,0,0.03)', border: '1px solid #f0e6dc' }}>
              <div style={{ color: '#5a4a3a', fontSize: '15px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span><span style={{ fontWeight: 600, color: '#ff5a00' }}>{totalProducts}</span> طقم بوركيني</span>
            ...
            {products.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '80px 20px', background: 'white', borderRadius: '20px', boxShadow: '0 10px 30px rgba(0,0,0,0.05)', border: '1px solid #f0e6dc' }}>
                ...
                <h3 style={{ fontSize: '24px', color: '#2c2418', marginBottom: '10px', fontWeight: 600 }}>لا يوجد بوركيني</h3>
                      ...
                      return (
                        <Link href={getProductUrl(p)} key={p._id} style={{ background: 'white', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 5px 20px rgba(0,0,0,0.05)', transition: 'all 0.3s', textDecoration: 'none', color: 'inherit', border: '1px solid #f0e6dc', display: 'block' }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-8px)'; e.currentTarget.style.boxShadow = '0 15px 30px rgba(255,90,0,0.12)'; e.currentTarget.style.borderColor = '#ff5a00' }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = '0 5px 20px rgba(0,0,0,0.05)'; e.currentTarget.style.borderColor = '#f0e6dc' }}>
                          <div style={{ position: 'relative', height: '280px', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #fef9f2 0%, #faf5ed 100%)' }}>
                            <img src={getProductImage(p.mainImage, p.imageUrl, { width: 400 }, p.images)} alt={p.name_ar || p.name_en || p.name_fr || 'بوركيني'} style={{ maxWidth: '90%', maxHeight: '90%', objectFit: 'contain', transition: 'transform 0.5s' }} loading="lazy" onError={(e) => { (e.target as HTMLImageElement).src = '/images/default.webp' }} />
                            {badges.length > 0 && <div style={{ position: 'absolute', top: '15px', right: '15px', display: 'flex', gap: '6px', flexDirection: 'column' }}>{badges}</div>}
                          </div>
                          <div style={{ padding: '20px', textAlign: 'center' }}>
                            <div style={{ fontSize: '11px', color: '#ff5a00', marginBottom: '8px', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '1px' }}>بوركيني</div>
      ...
      <section style={{ background: '#fef9f2', padding: '60px 0', borderTop: '1px solid #f0e6dc', marginTop: '40px' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
          <div style={{ maxWidth: '900px', margin: '0 auto' }}>
            <h2 style={{ fontSize: '32px', color: '#2c2418', marginBottom: '20px', fontWeight: 700, textAlign: 'center', fontFamily: 'Playfair Display, serif' }}>مجموعة البوركيني التركي</h2>
            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', marginBottom: '25px', textAlign: 'justify' }}>
              اكتشفي مجموعتنا من <strong>البوركيني التركي</strong>، ملابس السباحة المحجبات المصممة للراحة والوظائف 
              في الماء. تشمل مجموعتنا بالجملة أكثر من <strong>{totalProducts} طقم بوركيني</strong> بما في ذلك خيارات من قطعتين وقطعة واحدة. 
              هذه الملابس مصنوعة من <strong>أقمشة سريعة الجفاف ومقاومة للكلور</strong> توفر حماية من الأشعة فوق البنفسجية.
            </p>
            
            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>أنواع البوركيني المتاحة للشراء بالجملة</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '35px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>بوركيني من قطعتين</strong> – توب وبنطلون، سهل الارتداء</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>بوركيني من قطعة واحدة</strong> – مايوه كامل بتغطية شاملة</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>حجاب سباحة</strong> – غطاء رأس متناسق للطقم الكامل</span></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#ff5a00" strokeWidth="2"><path d="M20 6L9 17l-5-5" stroke="currentColor" fill="none"/></svg><span style={{ color: '#4a3a2a' }}><strong>بوركيني مقاسات كبيرة</strong> – مقاسات ممتدة حتى 4XL</span></div>
            </div>

            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>جدول المقاسات للبوركيني التركي</h3>
            <div style={{ background: 'white', borderRadius: '20px', padding: '30px', marginBottom: '35px', border: '1px solid #f0e6dc', boxShadow: '0 5px 20px rgba(0,0,0,0.03)' }}>
              <p style={{ textAlign: 'center', color: '#7a6a5a', marginBottom: '25px', fontSize: '14px' }}>المقاسات بالسنتيمتر (سم) - مقاسات الجسم لملابس السباحة</p>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'center' }}>
                  <thead>
                    <tr style={{ background: '#fef9f2', borderBottom: '2px solid #ff5a00' }}>
                      ...
                      <th style={{ padding: '15px 10px', fontWeight: 700, color: '#2c2418' }}>طول التونيك (سم)</th>
                  ...
                  <tbody>
                    ...
                    <td style={{ padding: '12px 10px', fontWeight: 600, color: '#ff5a00' }}>L/XL</td>
                    <td style={{ padding: '12px 10px', color: '#5a4a3a' }}>94-104</td>
                    <td style={{ padding: '12px 10px', color: '#5a4a3a' }}>74-84</td>
                    <td style={{ padding: '12px 10px', color: '#5a4a3a' }}>100-110</td>
                    <td style={{ padding: '12px 10px', color: '#5a4a3a' }}>70-75</td>
                    <td style={{ padding: '12px 10px', color: '#5a4a3a' }}>12-18</td>
                  </tr>
                  <tr style={{ borderBottom: '1px solid #f0e6dc' }}>
                    <td style={{ padding: '12px 10px', fontWeight: 600, color: '#ff5a00' }}>2XL/3XL</td>
                    <td style={{ padding: '12px 10px', color: '#5a4a3a' }}>104-116</td>
                    <td style={{ padding: '12px 10px', color: '#5a4a3a' }}>84-96</td>
                    <td style={{ padding: '12px 10px', color: '#5a4a3a' }}>110-120</td>
                    <td style={{ padding: '12px 10px', color: '#5a4a3a' }}>70-75</td>
                    <td style={{ padding: '12px 10px', color: '#5a4a3a' }}>20-26</td>
                  </tr>
              ...
              <p style={{ fontSize: '12px', color: '#a08c7a', textAlign: 'center', marginTop: '20px', fontStyle: 'italic' }}>*أقمشة البوركيني مطاطية. للحصول على أفضل مقاس، راجعي أدلة المقاسات الخاصة بالمنتج.</p>
            </div>

            <h3 style={{ fontSize: '24px', color: '#2c2418', margin: '35px 0 20px', fontWeight: 600 }}>الميزات الرئيسية للبوركيني التركي</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '15px', marginBottom: '35px' }}>
              <div style={{ padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><span style={{ fontWeight: 700, color: '#ff5a00' }}>جفاف سريع:</span><p style={{ color: '#4a3a2a', marginTop: '5px', fontSize: '14px' }}>يجف بسرعة بعد السباحة</p></div>
              <div style={{ padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><span style={{ fontWeight: 700, color: '#ff5a00' }}>مقاوم للكلور:</span><p style={{ color: '#4a3a2a', marginTop: '5px', fontSize: '14px' }}>يحافظ على اللون والشكل في حمامات السباحة</p></div>
              <div style={{ padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><span style={{ fontWeight: 700, color: '#ff5a00' }}>حماية من الأشعة فوق البنفسجية:</span><p style={{ color: '#4a3a2a', marginTop: '5px', fontSize: '14px' }}>حماية UPF 50+ في الموديلات المختارة</p></div>
              <div style={{ padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><span style={{ fontWeight: 700, color: '#ff5a00' }}>تغطية كاملة:</span><p style={{ color: '#4a3a2a', marginTop: '5px', fontSize: '14px' }}>أكمام طويلة، بنطلون بطول الكاحل</p></div>
              <div style={{ padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><span style={{ fontWeight: 700, color: '#ff5a00' }}>خفيف الوزن:</span><p style={{ color: '#4a3a2a', marginTop: '5px', fontSize: '14px' }}>لا يثقل في الماء</p></div>
              <div style={{ padding: '12px 16px', background: 'white', borderRadius: '12px', border: '1px solid #f0e6dc' }}><span style={{ fontWeight: 700, color: '#ff5a00' }}>مقاس مرن:</span><p style={{ color: '#4a3a2a', marginTop: '5px', fontSize: '14px' }}>أقمشة مطاطية للراحة أثناء الحركة</p></div>
            </div>

            <p style={{ fontSize: '16px', color: '#5a4a3a', lineHeight: '1.8', textAlign: 'justify', marginTop: '20px' }}>
              يوفر البوركيني التركي حلولاً عملية للنساء اللواتي يبحثن عن ملابس سباحة محجبات. هذه الملابس مصممة 
              <strong>بأقمشة سريعة الجفاف ومقاومة للكلور</strong> تؤدي أداءً جيداً في كل من حمامات السباحة ومياه البحر. 
              يوفر تصميم التغطية الكاملة الراحة والثقة أثناء السباحة والأنشطة الشاطئية. تشمل مجموعتنا بالجملة مقاسات 
              وأنماط مختلفة لتناسب مختلف الأذواق. تصفحي مجموعة البوركيني وقدمي لعملائك ملابس سباحة محجبات عملية ومريحة.
            </p>
          </div>
        </div>
      </section>
